...
class AdminController {
  static adminRegistration = async (req, res) => {
    try {
      ...
      if (admin)
        return res
          .status(400)
          .json({ success: false, message: "Aleady Register." });
    ...
    try {
      ...
      if (client)
        return res.status(400).json({
          success: false,
          message: "Client already present with this email.",
        });
      ...
      const html = `<!DOCTYPE html>
            ...
            <body>
              <div class="container">
                ...
                <br></br>
                <p>Regards,</p>
                <p>MeetingWithDevelopers</p>
                <span>Aditya Kumar Dixit </span>
              </div>
            </body>
            </html>
            `;
  ...
  static createClientForScript = async (req, res) => {
    try {
      ...
      if (req.params.token !== process.env.ADMIN_CLIENT_TOKEN)
        return res
          .status(400)
          .json({ success: false, message: "Unauthorized." });
  ...
  static updateClientApprove = async (req, res) => {
    try {
      ...
      const html = `
            ...
            <body>
                <div class="container">
                    ...
                    <p>Regards,</p>
                    <p class="signature">MeetingWithDevelopers,</p>
                    <p class="signature">Aditya Kumar Dixit</p>
                </div>
            </body>
            </html>
      ...
      if (client.approve === true) {
        sendMail(
          client.email,
          `CEOITBOX | Account Approval | ${client.name}`,
          html
        );
      }
      const clients = await ClientSchema.find({});
  ...
  static getUserDetailsToScheduling = async (req, res) => {
    try {
      ...
      const users = await User.find({}).select(
        "-avatar -refresh_token -_id -approve -weekdays -timeSlots -event -events -calendars -holidays -expertIn -registered"
      );
